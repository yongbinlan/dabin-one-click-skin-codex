...
foreach ($record in $rawProcesses) {
  ...
  # Codex desktop starts a separately cached CLI app-server. It is not another desktop.
  $cliRoot = Join-Path $env:LOCALAPPDATA 'OpenAI\Codex\bin'
  $isCliPath = $path.StartsWith($cliRoot + '\', [StringComparison]::OrdinalIgnoreCase)
  if ([string]$record.Name -ieq 'codex.exe' -and $isCliPath -and [string]$record.CommandLine -match '\bapp-server\b') {
    $parentRecord = $rawProcesses | Where-Object { [int]$_.ProcessId -eq $parentPid } | Select-Object -First 1
    if ($null -ne $parentRecord -and -not [string]::IsNullOrWhiteSpace([string]$parentRecord.ExecutablePath)) {
      $parentOwner = [pscustomobject]@{ Id=$parentPid; Path=[string]$parentRecord.ExecutablePath; ProcessName=[string]$parentRecord.Name }
      if (Test-CdpOwnerMatchesApp -Owner $parentOwner -App $app) { continue }
    }
  }
...
export async function queryWindowsRuntimeSnapshot({
  ...
  const { stdout, stderr = "" } = await execFileImpl(powershellPath, [
    ...
    "-EncodedCommand",
    Buffer.from(
      "[Console]::OutputEncoding = [System.Text.UTF8Encoding]::new($false); $ProgressPreference = 'SilentlyContinue'; " + WINDOWS_RUNTIME_SCRIPT + " " +
      [commonScriptPath, String(port), identityToken ?? ""]
